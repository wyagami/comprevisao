...
df = pd.DataFrame(columns=['Produto','Unitario','Descrição','Qtde','Valor'], widths =[10,10,100,10,10])
      ...
      for n in range(len(words)):
        ...
        for cutoff in list_cutoff:
          ...
          if similarity_sentence_text_bigram >= similarity_sentence_text_bigram_ant and similarity_sentence_text_bigram  > 0 and similarity_sentence_text_bigram  >= cutoff:
            insert(df,['{:<40}'.format(str(w2)),'{:>9}'.format(str(w22)),'{:<40}'.format(str(w1)),'{:>13}'.format(str(w11)),'{:>13}'.format(str(float(w11)*float(w22)))])
